...
class VisionWorker:
    def __init__(self, fieldConf: FieldConfig, camConf: CameraConfig, pipConf: PipelineConfig):
        ...
        videoMode = cscore.VideoMode(pixelFormat_=pixelFormat,width_=camConf.xres,height_=camConf.yres,fps_=camConf.fps)
    ...
    def run(self) -> None:
        ...
        while self._running:
            ...
            if perf_counter() - lastFPSTimestamp >= 1.0:
                print(f"{self._pipConf.name} running at {frameCounter} fps")
